...
export default class ObjectTransformer implements TransformerPlugin<Record<string, unknown>, Record<string, unknown>, Config> {
  ...
  async transform({ input, config }: ActionArgs<Record<string, unknown>, Config>): Promise<Record<string, unknown>> {
    if (!this.config) {
    ...
    for (const [outputField, template] of Object.entries(this.config.mappings)) {
